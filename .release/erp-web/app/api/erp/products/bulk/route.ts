...
import type { BulkProductMutationResult } from '@/app/lib/erp-types';
import { parseBulkProductRequest } from '@/app/lib/product-validation';
import { authenticateRequest, isAllowedOrigin, authorizeTenant } from '@/app/lib/request-access';
import { applyBulkProductMutation, createBulkProductFingerprint } from '@/db/product-repository';

// A fully populated 10,000-row update can approach 39 MB when valid XLSX text
// round-trips as JSON-escaped control characters. Keep bounded headroom above
// that parser-valid maximum.
...
function failureBody(message: string, total = 0): BulkProductMutationResult {
  return {
    ok: false,
    message,
...
export async function POST(request: Request) {
  ...
  if (!idempotencyKey || idempotencyKey.length < 8 || idempotencyKey.length > 128) {
    return NextResponse.json(
      failureBody('8~128자의 Idempotency-Key가 필요합니다.'),
      { status: 400, headers: corsHeaders(request) },
    );
  }
  if (request.headers.get('content-type')?.split(';', 1)[0]?.trim().toLowerCase() !== 'application/json') {
    return NextResponse.json(
      failureBody('Content-Type은 application/json이어야 합니다.'),
  ...
  if (contentLength && Number(contentLength) > MAX_JSON_BODY_BYTES) {
    return NextResponse.json(
      failureBody(MAX_JSON_BODY_MESSAGE),
  ...
  try {
    ...
    try {
      ...
      // The normalized parser result is the only representation needed below.
      // Drop the potentially 48 MiB raw JSON before hashing or awaiting D1.
      body.text = '';
    }
    if (typeof decoded === 'object' && decoded !== null && 'rows' in decoded && Array.isArray(decoded.rows)) {
      knownTotal = decoded.rows.length;
    }
    const parsed = parseBulkProductRequest(decoded);
    decoded = undefined;
    if (!parsed.ok) {
    ...
    // Hash only after raw/decoded representations have been released. The
    // repository receives the normalized fingerprint and does not stringify
    // the 10,000-row request a second time while those copies are retained.
    const fingerprint = await createBulkProductFingerprint(parsed.value);
    const result = await applyBulkProductMutation(
      parsed.value,
      idempotencyKey,
    ...
    console.error('Bulk product mutation failed', error);
    return NextResponse.json(
      failureBody('상품 일괄 처리 중 오류가 발생했습니다.', knownTotal),
