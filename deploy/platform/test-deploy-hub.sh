  ...
  case "$object:$operation" in
    ...
    service:inspect)
      ...
      case "$*" in
        ...
        *) test "$target" = 'starsnap-log_web' ;;
      esac
      ;;
    service:ls)
      printf 'starsnap-log_web 1/1\n'
      ;;
    service:update)
      while (( $# > 0 )); do
        case "$1" in
...
FAKE_HUB_WEB_IMAGE="ghcr.io/starsnap/starsnap-log-web-runtime@sha256:$(printf 'a%.0s' {1..64})"
FAKE_HUB_WEB_PULL_IMAGE='ghcr.io/starsnap/starsnap-log-web-runtime:release-test'
...
wrong_digest="ghcr.io/starsnap/starsnap-log-web-runtime@sha256:$(printf 'b%.0s' {1..64})"
...
assert_contains "$rollback_health_output" 'The restored starsnap-log_web task is not core-healthy.'
